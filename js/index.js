...
function createProductElement(product, index) {
  ...
  if (product.onSale) {
    ...
    const clonedCartBtn = clonedNode.querySelector(".cart");
    if (clonedCartBtn) {
      clonedCartBtn.addEventListener("click", function () {
        alertContainer.classList.remove("d-none");
      });
    }
  }
...
const registerCloseBtn = document.querySelector(".register-container .close");
const registerBtn = document.querySelector(".user-nav .register-btn");
...
const loginCloseBtn = document.querySelector(".login-container .close");
const loginBtn = document.querySelector(".user-nav .login-btn");
...
function newUser() {
  if (
    ...
    registerFormValidation(registerEmailInput, "inValidRegisterEmail") &&
    registerFormValidation(telePhoneInput, "inValidPhoneNumber") &&
    registerFormValidation(registerPasswordInput, "inValidPassword") &&
    confirmPassword() &&
    emailValidation()
  ) {
    const user = {
...
function registerFormValidation(input, inValidMsg) {
  const regex = {
    userName: /^[A-Za-z]{3,20}(?:[ _-][A-Za-z]{3,20})*$/,
    registerEmail:
      /^[a-zA-Z][a-zA-Z0-9._-]+@[a-zA-Z][a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/,
    telephone: /^(\+2)?01[0125][0-9]{8}$/,
    registerPassword:
...
function emailValidation() {
  const inValidEmailMsg = document.getElementById("alreadyExist");
  let emailExists = false;

  for (let i = 0; i < userList.length; i++) {
    if (registerEmailInput.value === userList[i].email) {
      emailExists = true;
      break;
    }
  }
  ...
  if (emailExists) {
    inValidEmailMsg.classList.remove("d-none");
    return false;
  } else {
    inValidEmailMsg.classList.add("d-none");
    return true;
  }
}
function confirmPassword() {
  const confrimPasswordMsg = document.getElementById("inValidConfirmPassword");
  if (confirmPasswordInput.value !== registerPasswordInput.value) {
    confrimPasswordMsg.classList.remove("d-none");
    confirmPasswordInput.classList.add("is-invalid");
    confirmPasswordInput.classList.remove("is-valid");
    return false;
  } else {
    confrimPasswordMsg.classList.add("d-none");
...
function loginCheck() {
  ...
  if (
    admin.userName === loginEmail.value.trim() &&
    admin.password === loginPassword.value.trim()
  ) {
